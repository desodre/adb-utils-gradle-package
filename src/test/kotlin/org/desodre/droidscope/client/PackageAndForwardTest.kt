...
import kotlin.test.*
import org.desodre.droidscope.error.PackageOperationException
...
class PackageAndForwardTest {
    ...
    @Test fun `install uses SYNC package manager and cleanup sessions`() = runBlocking<Unit> {
        ...
        assertTrue(device.install(byteArrayOf(1, 2), InstallOptions(grantRuntimePermissions = true)).success)

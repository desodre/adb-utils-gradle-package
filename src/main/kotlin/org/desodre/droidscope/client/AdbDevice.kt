...
import org.desodre.droidscope.protocol.ShellV2Protocol
import org.desodre.droidscope.protocol.SyncProtocol
...
class AdbDevice internal constructor(private val client: AdbClient, val serial: DeviceSerial) {
    ...
    suspend fun install(apk: ByteArray, options: InstallOptions = InstallOptions()): InstallResult {
        ...
        return try {
            ...
            InstallResult(result.exitCode == 0 && message.lineSequence().any { it.trim() == "Success" }, message)
        } finally {
            runCatching { shellV2("rm -f '$remote'") }
        }
    }
    ...
    internal suspend fun selectTransport(protocol: org.desodre.droidscope.protocol.AdbProtocol) {
